...
export async function GET(request) {
  ...
  try {
    ...
    console.error('Error in GET /api/settings/about:', error);
    return NextResponse.json({ error: "เกิดข้อผิดพลาดในการดึงข้อมูลการตั้งค่า" }, { status: 500 });
  }
}
...
export async function PUT(request) {
  try {
    ...
    if (checkError && checkError.code === 'PGRST116') {
      ...
      if (insertError) {
        console.error('Error creating about settings:', insertError);
    ...
    console.error('Error in PUT /api/settings/about:', error);
    return NextResponse.json({ error: "เกิดข้อผิดพลาดในการอัปเดตการตั้งค่า" }, { status: 500 });
  }
}
